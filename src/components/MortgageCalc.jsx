...
const TR = {
  sq: {
    amount: 'Shuma e huasë', rate: 'Norma vjetore (%)', years: 'Afati (vjet)',
    selectBank: 'Zgjidhni bankën', customRate: 'Normë manuale',
    monthly: 'Kësti mujor', total: 'Totali i paguar', interest: 'Interesi total',
    income: 'Paga minimale e nevojshme', incomeNote: 'Kësti ≤ 35% e pagës mujore',
    compare: 'Krahasim i bankave',
    morePerMonth: '/muaj më shumë', best: 'Më e mira',
    note: 'Llogaritja është indikative. Kontaktoni bankën për ofertë zyrtare.',
    yr: 'vj',
  },
  en: {
    amount: 'Loan amount', rate: 'Annual rate (%)', years: 'Term (years)',
    selectBank: 'Select bank', customRate: 'Custom rate',
    monthly: 'Monthly payment', total: 'Total paid', interest: 'Total interest',
    income: 'Minimum income required', incomeNote: 'Payment ≤ 35% of monthly income',
    compare: 'Bank comparison',
    morePerMonth: '/month more', best: 'Best',
    note: 'Calculation is indicative. Contact the bank for an official offer.',
    yr: 'yr',
  },
  it: {
    amount: 'Importo mutuo', rate: 'Tasso annuo (%)', years: 'Durata (anni)',
    selectBank: 'Seleziona banca', customRate: 'Tasso manuale',
    monthly: 'Rata mensile', total: 'Totale pagato', interest: 'Interessi totali',
    income: 'Reddito minimo richiesto', incomeNote: 'Rata ≤ 35% del reddito mensile',
    compare: 'Confronto banche',
    morePerMonth: '/mese in più', best: 'Migliore',
    note: 'Il calcolo è indicativo. Contatta la banca per un\'offerta ufficiale.',
    yr: 'a',
  },
};

function calcMortgage(amount, annualRatePct, years) {
  if (!amount || !years) return null;
  if (annualRatePct === 0) {
    const m = amount / (years * 12);
...
export default function MortgageCalc({ banks = [], currency = 'ALL', lang = 'sq' }) {
  const tr = TR[lang] ?? TR.sq;
  const defaultAmount = currency === 'ALL' ? 5000000 : 50000;

  // ── ALL HOOKS BEFORE ANY RETURN ──────────────────────
  const [amount, setAmount]         = useState(defaultAmount);
  const [rate, setRate]             = useState(banks[0]?.mortgageRatePct ?? 5.5);
  ...
  const handleBankChange = useCallback((name) => {
    ...
    if (bank) setRate(bank.mortgageRatePct);
  }, [banks]);

  const MAX_MORTGAGE = 100_000_000_000; // 100 billion
  const handleAmountInput = useCallback((e) => {
    const raw = e.target.value.replace(/[^\d]/g, '');
    const n = Number(raw) || 0;
    setAmount(Math.min(n, MAX_MORTGAGE));
  }, []);
  // ─────────────────────────────────────────────────────
  ...
  const result = calcMortgage(amount, rate, years);
  const bestRate = banks.length ? Math.min(...banks.map(b => b.mortgageRatePct)) : rate;
  const requiredIncome = result ? result.monthly / 0.35 : 0;
  ...
  return (
    <div className="mc-wrap">
      ...
      <div className="mc-inputs">
            ...
            <div className="mc-bank-pills">
              {banks.map(b => (
                ...
                >
                  ...
                  <span className="mc-bank-pill-rate">{b.mortgageRatePct.toFixed(2)}%</span>
                </button>
              ))}
              <button
        ...
        {/* Years */}
        <div className="mc-field">
          <label className="mc-label">{tr.years}</label>
          <div className="mc-years-grid">
            {YEARS.map(y => (
              <button
                key={y}
                className={`mc-year-btn${years === y ? ' mc-year-btn--active' : ''}`}
                onClick={() => setYears(y)}
              >
                {y} {tr.yr}
              </button>
      ...
      {result && (
        <div className="mc-result-panel">
          ...
          <div className="mc-breakdown">
            ...
            <div className="mc-breakdown-row mc-breakdown-row--income">
              <span>{tr.income} <small>({tr.incomeNote})</small></span>
              <strong>{fmt(requiredIncome, currency)}</strong>
            </div>
          </div>

          {/* Interest bar visualizer */}
          {result.totalPaid > 0 && (
            <div className="mc-bar-wrap" title="Principal vs Interest">
              <div
                className="mc-bar-principal"
                style={{ width: `${(amount / result.totalPaid * 100).toFixed(1)}%` }}
              />
          ...
          {banks.length > 1 && (
            <div className="mc-compare">
              ...
              <div className="mc-compare-list">
                {banks.map(b => {
                  const r = calcMortgage(amount, b.mortgageRatePct, years);
                  const diff = r ? r.monthly - result.monthly : 0;
                  const isBest = b.mortgageRatePct === bestRate;
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="mc-cmp-rate">{b.mortgageRatePct.toFixed(2)}%</span>
                      {r && (
